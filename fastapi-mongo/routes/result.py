...
from database.database import retrieve_exam_list_result, retrieve_station_result, retrieve_exam_result
...
@router.get("/exam/{session_id}/station/{station_id}")
async def get_station_result(session_id: PydanticObjectId, station_id: PydanticObjectId):
    exam_results = await retrieve_station_result(session_id, station_id)
    if not exam_results:
        raise HTTPException(status_code=404, detail="Không tìm thấy kết quả thi")
    return exam_results
